...
db = np.arange (68, 92, 1)
print (peak)
# 4. Plot heatmap 
plt.figure(figsize=(10, 6))
plt.plot (degrees, magnitude[row_num,:])
plt.ylim (bottom=68, top=92)
plt.xlabel('Azimuth gradi')
plt.ylabel('Intensità [dB]')
plt.title('Azimuth al range del target a ' + str( round(row_num*radar_res, 4)) + ' metri')
plt.tight_layout()
plt.grid(which = "both",linestyle="--")
plt.vlines (x = 0, linestyle="--", ymin = 68, ymax = 92, label="0° gradi")
